...
pub enum FileCommands {
    ...
    /// Create or manage direct download links
    ///
    /// V3 API: use file IDs (e.g., "abc123")
    /// V4 API: use file paths (e.g., "/folder/file.txt")
    Link {
        /// File ID(s) for V3 or file path(s) for V4
        #[clap(short = 'f', long, required = true, num_args = 1..)]
        file: Vec<String>,

        /// Delete a direct link by ID (V4 only)
        #[clap(short, long)]
...
pub async fn handle_file_command(client: &CloudreveAPI, command: FileCommands) -> Result<()> {
    match command {
        ...
        FileCommands::Link { file, delete } => match client.inner() {
            UnifiedClient::V4(v4_client) => {
                if let Some(id) = delete {
                    link::handle_delete_link_v4(v4_client, id).await
                } else {
                    link::handle_create_link_v4(v4_client, file).await
                }
            }
            UnifiedClient::V3(v3_client) => {
                if delete.is_some() {
                    Err(cloudreve_api::Error::InvalidResponse(
                        "Deleting direct links is not supported for V3 API".to_string(),
                    ))
                } else {
                    link::handle_create_link_v3(v3_client, file).await
                }
            }
        },
    }
